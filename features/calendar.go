...
func PrintCal(regNo string, cookies types.Cookies, sem_choice int, classGrpFlag int) {
    ...
    processDates(regNo, cookies, semester, grp_list, datelist)
}
...
func processDates(regNo string, cookies types.Cookies, semester types.Semester, grp_list [][]string, datelist []string) {
    var months []string
    year := datelist[0][7:]
    var color_list [][]int
    ...
    for _, date := range datelist {
        ...
        typeOfDay := extractTypeOfDay(doc)
...
func extractTypeOfDay(doc *goquery.Document) []int {
    var typeOfDay []int
